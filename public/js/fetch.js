...
const request = async (url, params, method = 'GET') => {
    ...
    if (response.status === 404 || response.status === 500) {
        Swal.fire({ icon: 'error', title: 'Oops...', showConfirmButton: false, timer: 3000, text: 'The server responded with an unexpected status!', footer: '<a href>NEUST MIS</a>' })
...
const updateOrCreate = async (url, params, method = 'POST') => {
    ...
    if (res) { Swal.fire({ position: 'top-end', icon: 'success', title: msge, showConfirmButton: false, timer: 1500, footer: '<a href>NEUST MIS</a>' }); return res; }
}
const destroy = async (_entity, pk, willRemove = true) => {
    const { value: result } = await Swal.fire({ title: 'Are you sure?', text: "You won't be able to revert this!", icon: 'warning', showCancelButton: true, confirmButtonColor: '#3085d6', cancelButtonColor: '#d33', confirmButtonText: 'Yes, delete it!', footer: '<a href>NEUST MIS</a>' })
    if (result) {
        ...
        if (response) {
            if (response.success) {
                ...
                Swal.fire({ position: 'top-end', icon: 'success', title: 'Your work has been saved', showConfirmButton: false, timer: 1500, footer: '<a href>NEUST MIS</a>' })
            }
